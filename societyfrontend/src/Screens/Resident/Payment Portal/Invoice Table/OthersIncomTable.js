...
export default function OtherIncomeTable({ userInfo }) {
  ...
  const handleOpen = async (id) => {
    const result = await apiHelper.getEventDetailsById(id)
  ...
  const downloadInvoice = () => {
    ...
    const dueDate = MaintenanceDetails?.eventId?.dueDate?.split("T")[0] || "Date";
  ...
  let total = Math.floor(MaintenanceDetails?.amount / Member?.length)
  if (total % 10 !== 0) {
  ...
  return <>
    <Paper elevation={5} className='p-2'>
      <div className="d-flex align-items-center justify-content-between mb-3 mt-1">
        <div className="h6 fw-bold mt-2">Event Invoices List</div>
        <div>
          {/* <button className='btn btn_primary fw-bold' onClick={() => navigate(Path.MemberRegister)} >View Invoice</button> */}
        </div>
      </div>
      <TableContainer>
    ...
    >
      <Box sx={style}>
        <div id="invoice-content">
          ...
          <Typography sx={{ mt: 2 }}>
            <Paper id="invoice-content" elevation={5} style={{ background: 'whitesmoke', padding: '10px' }}>
              <div className="my-2">
              ...
              <div className="d-flex justify-content-between my-2">
                <div className="">
                  ...
                  <div className="">{MaintenanceDetails?.eventId?.dueDate?.split("T")[0]}</div>
                </div>
                <div className="">
            ...
            <Paper elevation={5} style={{ background: 'whitesmoke', padding: '10px', marginTop: '12px' }}>
              <div className="d-flex justify-content-between">
                <div className="">Event Total Amount</div>
                <div className="text-success">₹ {MaintenanceDetails?.amount}</div>
              </div>
              <div className="d-flex justify-content-between">
                <div className="">Per Member Amount</div>
                <div className="text-primary">₹ {Math.floor(MaintenanceDetails?.amount / Member?.length)}</div>
              </div>
              <hr />
              <div className="d-flex justify-content-between">
